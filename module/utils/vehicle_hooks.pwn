// Vehicle Hooks Handle (imported from EVF.inc by Cipher)
// hybrid of callbacks.inc (Vehicle Function related - Spraying Can)
native IsValidVehicle(vehicleid);

enum E_DATA_VEHICLE_PROPERTY
{
	e_vehicleInterior,
	e_vehiclePaintjob,
	e_vehicleColor1,
...
new VEHICLE_PROPERTY[MAX_VEHICLES][E_DATA_VEHICLE_PROPERTY];

static const DATA_CarColors[][]={
...
forward OnVehicleDestroyed(vehicleid, Float:x, Float:y, Float:z);
forward OnPlayerSprayAtVehicle(playerid, vehicleid);

// Hooks Function 

stock _gtaprp_DestroyVehicle(vehicleid)
{
	new Float:x, Float:y, Float:z;
	GetVehiclePos(vehicleid, x, y, z);
	CallLocalFunction("OnVehicleDestroyed", "dfff", vehicleid, x, y, z);

	VEHICLE_PROPERTY[vehicleid][e_vehicleColor1] = VEHICLE_PROPERTY[vehicleid][e_vehicleColor2] = 0;
	VEHICLE_PROPERTY[vehicleid][e_vehiclePaintjob] = VEHICLE_PROPERTY[vehicleid][e_vehicleInterior] = 0;
...
#define DestroyVehicle _gtaprp_DestroyVehicle

stock _gtaprp_AddStaticVehicleEx(vehicletype, Float:x, Float:y, Float:z, Float:rotation, color1, color2, respawn_delay)
{
	new vehicleid = AddStaticVehicleEx(vehicletype, x, y, z, rotation, color1, color2, respawn_delay);
	VEHICLE_PROPERTY[vehicleid][e_vehicleColor1] = color1;
	VEHICLE_PROPERTY[vehicleid][e_vehicleColor2] = color2;
...
#define AddStaticVehicleEx _gtaprp_AddStaticVehicleEx

stock _gtaprp_AddStaticVehicle(vehicletype, Float:x, Float:y, Float:z, Float:rotation, color1, color2)
{
...
#define AddStaticVehicle _gtaprp_AddStaticVehicle

stock _gtaprp_CreateVehicle(vehicletype, Float:x, Float:y, Float:z, Float:rotation, color1, color2, respawn_delay, bool:addsiren = false)
{
...
#define CreateVehicle _gtaprp_CreateVehicle

stock _gtaprp_ChangeVehicleColor(vehicleid, color1, color2)
{
...
#define ChangeVehicleColor _gtaprp_ChangeVehicleColor

stock _gtaprp_ChangeVehiclePaintjob(vehicleid, paintjobid)
{
	if(ChangeVehiclePaintjob(vehicleid, paintjobid))
	{
		VEHICLE_PROPERTY[vehicleid][e_vehiclePaintjob] = paintjobid;
		if(paintjobid == 3)
		{
			new color1, color2;
			if(GetVehicleColor(vehicleid, color1, color2))
			{
...
#define ChangeVehiclePaintjob _gtaprp_ChangeVehiclePaintjob

stock _gtaprp_LinkVehicleToInterior(vehicleid, interiorid)
{
	if(LinkVehicleToInterior(vehicleid, interiorid))
	{
		VEHICLE_PROPERTY[vehicleid][e_vehicleInterior] = interiorid; 
		return 1;
	}
	return 0;
}
...
#define LinkVehicleToInterior _gtaprp_LinkVehicleToInterior
...
public OnVehicleRespray(playerid, vehicleid, color1, color2)
{
	...
	#if defined VEHFIXES_OnVehicleRespray
		return VEHFIXES_OnVehicleRespray(playerid, vehicleid, color1, color2);
	#else
		return 1;
	#endif	
}
...
#endif
#define OnVehicleRespray VEHFIXES_OnVehicleRespray
#if defined VEHFIXES_OnVehicleRespray
	forward VEHFIXES_OnVehicleRespray(playerid, vehicleid, color1, color2);
#endif
...
stock GetVehicleRandomColors(modelid,&color1,&color2)
{
	...
	if(!DATA_CarColors[modelid][0]) return 0;
	
	if(DATA_CarColors[modelid][0] == 2){
		color1 = DATA_CarColors[modelid][1];
		color2 = DATA_CarColors[modelid][2];
	} else {
		index = random(DATA_CarColors[modelid][0]) & -2;
		color1 = DATA_CarColors[modelid][++index];
		color2 = DATA_CarColors[modelid][++index];
	}
	return 1;
}
...
stock GetVehicleColor(vehicleid, &color1, &color2)
{
	...
	{
		color1 = VEHICLE_PROPERTY[vehicleid][e_vehicleColor1];
		color2 = VEHICLE_PROPERTY[vehicleid][e_vehicleColor2];
		return 1;
	}
	return 0;
}
...
{
	return VEHICLE_PROPERTY[vehicleid][e_vehiclePaintjob];
}

stock GetVehicleInterior(vehicleid)
{
	return VEHICLE_PROPERTY[vehicleid][e_vehicleInterior];
}