...
/*	  X 	 Y 	     Z 		Angle */
	{217.0299,626.3177,899.9175,45.6326}, // {1041.7264,-902.4476,43.4689,313.9807}, // Red
    {208.3860,635.7946,899.9175,221.4822} // {1046.0197,-897.8440,43.4689,140.3924} // Blue
};
new Float:arrTalpakAreaPosition[6] = {
    213.1116, // 1043.9049, // Position X
    630.7337, // -899.7435, // Position Y
    899.9175, // 43.4689, // Position Z
    100.0, // Range
    1.0, // Interior World
    1.0 // Virual World
};
...
CreateTalpakGUI()
{
    gtd_TalpakUI[0] = TextDrawCreate(265.000000, 267.000000, "RED");
	...
	gtd_TalpakUI[1] = TextDrawCreate(365.000000, 267.000000, "BLUE");
...
TalpakPlayerUpdateBet(playerid)
{
    if(IsPlayerInDynamicArea(playerid, TalpakData[e_iTalpakArea]) && TalpakData[e_bTalpak] && GetPVarType(playerid, "TALPAKAN::Bet") && GetPVarInt(playerid, "TALPAKAN::Bet") > 0)
    {
        ...
        CreatePlayerTalpakGUI(playerid);
        format(szMessage, sizeof(szMessage), "You bet ~y~$%s~w~ on %s you will receive ~g~$%s (-%d%%)", number_boxformat(bet), (team == 1) ?  ("~r~Red~w~") : ("~b~Blue~w~") ,number_boxformat((bet * 2) - fee), difference);
...
ShowTalpakBetMenu(playerid, dialogid)
{
    ...
    switch(dialogid)
    {
        case DIALOG_BOXINGEVENT_MENU:
        {
            ...
            format(szTitle, sizeof(szTitle), "Asian Gaming Betting");
        ...
        case DIALOG_BOXINGEVENT_BET:
        {
            ...
            switch(type)
            {
                case 1:
                {
                    format(szDialog, sizeof(szDialog), "{FFFFFF}You are about to place a bet on %s {FF0000}Red{FFFFFF}!\n\
                    ...
                    format(szTitle, sizeof(szTitle), "Asian Town Betting");
                ...
                case 2:
                {
                    format(szDialog, sizeof(szDialog), "{FFFFFF}You are about to place a bet on %s {0000FF}Blue{FFFFFF}!\n\
                        Minimum Bet:{FFFFFF} $5,000\nMaximum Bet:{FFFFFF} $500,000\n\
                        {FFFF00}----------------------------------------\n\
                        {FF0000}Note: Once you lock your bet you won't able to turn back!\nWe're not responsible of your bet once you get DISCONNECTED from the Server so make sure\nBet Responsibly.\n\n\
                        Total of Bet:{FFFFFF} $%s\nNumber of Bet:{FFFFFF} %s\n\n{FFFF00}Please input your amount of bet below.",
...
EndTalpakEvent()
{
    if(IsPlayerConnected(TalpakData[e_iRedPlayerid]))
    {
        ...
        SetPlayerHealth(TalpakData[e_iRedPlayerid], GetPVarFloat(TalpakData[e_iRedPlayerid], "TALPAKAN::OldHealth"));
    ...
    if(IsPlayerConnected(TalpakData[e_iBluePlayerid]))
    {
        ...
        SetPlayerHealth(TalpakData[e_iBluePlayerid], GetPVarFloat(TalpakData[e_iBluePlayerid], "TALPAKAN::OldHealth"));
...
public TalpakDelayTick()
{
    ...
    if(TalpakData[e_iStartTime] <= 0)
    {
        foreach(new playerid : Player)
        {
            ...
            {
                format(szMessage, sizeof(szMessage), "~y~Match Start!~n~~r~Red~w~ VS ~b~Blue~n~~g~Fight!");
    ...
    foreach(new playerid : Player)
    {
        if(IsPlayerInDynamicArea(playerid, TalpakData[e_iTalpakArea]))
        {
            format(szString, sizeof(szString), "~y~Match between~n~~r~Red~w~ VS ~b~Blue~n~~y~%d~w~ seconds before it start.", TalpakData[e_iStartTime]);
...
CMD:startfight(playerid, params[])
{
	...
	if(sscanf(params, "uudddd", blueplayerid, redplayerid, delay))
		return SendClientMessage(playerid, COLOR_LIGHTBLUE, "Usage: "WHITE"/startfight [blue playerid] [red playerid] [delay]");
    ...
    new szMessage[128];
	format(szMessage, sizeof(szMessage), "%s as {FF0000}Red{FFFFFF} vs %s as {0000FF}Blue{FFFFFF} match starts in %d Seconds.", GetPlayerNameEx(TalpakData[e_iRedPlayerid]), GetPlayerNameEx(TalpakData[e_iBluePlayerid]), TalpakData[e_iStartTime]);
